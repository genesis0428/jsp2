...
<body>	<!-- 메인글: 익명으로 예시(글 비밀번호 사용합니다.) , 댓글 : 실명으로 예시(로그인을 해야 글 작성, 삭제할 수 있습니다.) -->
<main>
...
<!-- 				<li>글 비밀번호</li><!-- 댓글 삭제 : 1)로그인한 사용자가 본인글 삭제 또는 2)글 비밀번호 사용 			
			...
			<li>
				<ul style="display: flex;">
					...
					<!-- sessionScope.user 는 session에 저장된 애트리뷰트 user 를 가져옵니다. 
						 sessionScope 을 생략할 수 있지만 request, application 등 다른 영역에 중복된 이름이 있을 때를
						 고려하여 명시적으로 지정합니다.
					-->
					...
					<c:if test="${sessionScope.user == null }">	<!-- 로그인 안했을때 -->
			...
			<li>
				<ul class="crow">
					...
					<li>${cmt.ip }</li>				
					<li>${cmt.wdate }</li>
					<!-- 삭제 구현 방법 1) 로그인한 사용자의 글만 삭제버튼이 보이게 하기 -->				
...
//	let close = document.getElementsByClassName('close')[0];	//getElementsXXX 이면 배열 리턴. 그 요소중에 0번 인덱스 
	...
	//댓글 삭제 함수
		function deleteCmt(cmtidx,idx){
			...
			if(yn){
				//함수의 인자로 받은 2개의 값은 파라미터로 전달합니다. 추가되ㄴ 파라미터 del 이 있습니다.
		...
		function login(){
			...
			// 다시 지금 페이지로 돌아오기 위해 필요한 값 mref,page 요소 값을 전달.(textarea는 필요없음)
