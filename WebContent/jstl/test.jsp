...
<ul>
	...
	<li>jstl 과 el 은 함께 사용됩니다. (el 은 라이브러리 추가 없이 사용합니다.)</li>
</ul>
<h4>자바 변수처럼 값을 저장하기</h4>
<c:set var="age" value="13"/>
age : ${age} <br>				<!-- XSS 해킹공격에 취약한 형식 -->
age : <c:out value="${age }"/>	<!-- el 출력할 때 사용합니다. 엄격한 형식을 적용하는 방법 -->
<hr>
<!--  -->
...
<c:choose>
	...
	<c:when test="${age <23 }">		<!-- 첫번쨰 조건이 거짓일 때만 실행합니다. -->
...
<c:forEach var = "x" begin="10" end="30" step="2" varStatus="status"><!-- varStatus 현재 상태 저장할 변수 -->
...
<ul><c:forTokens items="${twice }" var="member" delims=",">
	<li>
